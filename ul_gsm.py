...
class Sms:
    ...
    def decode_in(self, pdu):
        ...
        if len(pdu) >= 25:
            ...
            self.firstOctet = pdu[i];
            i += 1

            self.senderLen = (pdu[i] >> 1) + (pdu[i] & 1);
            i += 1
            self.senderType = pdu[i];
            i += 1
            self.sender = phone_unpack(pdu[i: i + self.senderLen]);
            i += self.senderLen
            ...
            self.year, self.month, self.date, self.hour, self.minute, self.second, self.tz = date[0], date[1], date[2], \
